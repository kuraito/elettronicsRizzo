...
export default function Prodotti({ featuredProducts = [] }: { featuredProducts?: Product[] }) {
  return (
        ...
        {featuredProducts.length > 0 && (
          <div className="mt-12 sm:mt-16 reveal">
            <div className="text-center mb-8">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-100 text-amber-700 rounded-full text-xs font-semibold mb-3">
                <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
                </svg>
                Selezione del mese
              </span>
              <h3 className="text-xl sm:text-2xl font-bold text-gray-900">Prodotti in Evidenza</h3>
            </div>
            <div className={`grid gap-3 sm:gap-5 ${
              featuredProducts.length === 1 ? "grid-cols-2 sm:grid-cols-1 sm:max-w-xs sm:mx-auto" :
              featuredProducts.length === 2 ? "grid-cols-2 sm:grid-cols-2 sm:max-w-xl sm:mx-auto" :
              featuredProducts.length <= 4 ? "grid-cols-2 lg:grid-cols-4" :
              ...
              {featuredProducts.map((product) => (
                ...
                >
                  ...
                  <div className="relative h-28 sm:h-40 bg-gray-50">
                    ...
                    <span className="absolute top-2 left-2 inline-flex items-center gap-1 px-2 py-0.5 bg-amber-400 text-white text-xs font-semibold rounded-full">
                  ...
                  <div className="p-3 sm:p-4">
                    ...
                    <h4 className="font-semibold text-gray-900 text-sm mt-0.5 mb-1 line-clamp-2">{product.name}</h4>
                    <p className="text-primary-700 font-bold text-sm mb-3">{product.price}</p>
                    <a
                      href={`https://wa.me/393899937298?text=Ciao%2C%20vorrei%20informazioni%20su%3A%20${encodeURIComponent(product.name)}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-center gap-1.5 w-full py-2 bg-primary-50 hover:bg-primary-600 text-primary-700 hover:text-white rounded-xl text-xs font-semibold transition-all duration-300"
                    >
                      <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                      </svg>
